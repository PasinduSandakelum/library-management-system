...
public class AddMain extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddMain</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddMain at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        mainClass.setMid(mainId);
        mainClass.setMname(mainClassification);
        ...
        try {
            ...
            if (b) {
                ...
                request.setAttribute("error", "<div class=\"col-sm-8 col-sm-offset-2\">\n"
                        ...
                        + "                <a href=\"#\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a>\n"
                        + "                <strong>Warning</strong>Main ID already exists.!\n"
                        + "            </div>\n"
                        + "        </div>");
                request.getRequestDispatcher("/AddMainClass.jsp").forward(request, response);
            }
