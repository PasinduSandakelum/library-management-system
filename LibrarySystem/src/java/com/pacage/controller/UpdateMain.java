...
public class UpdateMain extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UpdateMain</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UpdateMain at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("type").equals("fill")) {
            ...
            request.setAttribute("mainName", mainName);
            request.getRequestDispatcher("/EditMain.jsp").forward(request, response);
            
        }else if(request.getParameter("type").equals("edit")){
            String mainId = request.getParameter("mainId");
            String mainName = request.getParameter("mainClassification");
            try {
                boolean test = mainDao.editMainClassification(mainId, mainName);
                if (test) {
                    request.setAttribute("error", "<div class=\"col-sm-8 col-sm-offset-2\">\n"
                        + "            <div class=\"alert alert-success alert-dismissible\">\n"
                        + "                <a href=\"#\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a>\n"
                        + "                <strong>Success</strong>Main classification was updated!<a href=\"SearchMain?type=all\"><span class=\"btn btn-md\" ><span class=\"glyphicon glyphicon-search\"></span>View All</span></a>\n"
                        + "            </div>\n"
                        + "        </div>");
                    request.getRequestDispatcher("/EditMain.jsp").forward(request, response);
                } else {
